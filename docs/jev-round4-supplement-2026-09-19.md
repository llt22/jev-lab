...
## 10. 社区实测归档：模型路由会破坏 prompt cache（Antonio Leiva）

> 用户点题后核实，2026-09-19；原始抓取见 [`antonio-levia-模型路由缓存实测-2026-09-19.json`](../artifacts/round4-2026-09-19/antonio-levia-模型路由缓存实测-2026-09-19.json)。

**Antonio Leiva（[@antonioleivag](https://x.com/antonioleivag)）**，西班牙马德里，dev.expert 创始人（Kotlin/MCP 讲师）。9-18 发了三条相关的帖子，其中一条是**本轮最有成本价值的一手实测**：

- **Jev 首次使用**：安装 [Codex Router](https://codex-router.com) 用 Jev 做 Codex 的模型路由（"Jev 承诺在这方面很有用"）。
- **🚨 实测结果**（[原帖](https://x.com/antonioleivag/status/2101086190363455551)）：同一个功能，**用路由器（切模型）时缓存命中率 32%，不切模型时 99.35%**——"这个机制确实会破坏缓存，而且破坏得很严重"。推理 API 对重复令牌的缓存计费约便宜 10–15 倍，切模型的收益必须能覆盖丢失的缓存折扣，否则不划算。他的结论："**如果您不太清楚自己在做什么，我建议避免使用它**"；并给出优化方向——把缓存损失惩罚值计入切换决策（"正在用 Astra 时切到 Luna 做几个任务"这类场景才值得）。
- **评论区的两个方向**：① 与其换模型不如换 effort 档位（Claude 里改 effort 似乎不使缓存失效，@husseincak）；② "给 Jev 找个用途似乎是最难的部分"（@ErickSky）。

**与本仓库实验的对照**：agent-control 的模型路由实验当时只验证了路由准确率（93.3%，胜过"全部强模型"但没胜过关键词规则），**没有测缓存成本**——Leiva 的实测补上了真实部署里最要命的成本维度，且结论与 Theo 对上下文压缩的第五点批评（缓存写入比读取昂贵）完全同构：**任何改变对话前缀/历史的机制（压缩、切模型）都会让缓存失效，路由收益必须大于缓存惩罚。**这应加入本项目模型路由实验的下一轮验证项。
